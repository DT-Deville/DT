...
// tries to auto-detect them and stops dt when any one
// of them is invalid.
func InitPaths() {
	...
	if isAppX {
		appDestPath = filepath.Join(dtFolder, "AppX")
	} else {
		appDestPath = appPath
	}
...
func GetConfigPath() string {
	return filepath.Join(dtFolder, "config-xpui.ini")
}
...
func CheckUpdate(version string) {
	if !settingSection.Key("check_dt_update").MustBool() || version == "Dev" {
		return
	}
	...
	if latestTag != version {
		...
		utils.PrintInfo(`Run "dt update" or use a package manager to update dt`)
	}
}
