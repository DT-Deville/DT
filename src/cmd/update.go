...
func Update(currentVersion string) bool {
	...
	if currentVersion == tagName {
		utils.PrintSuccess("DT is up-to-date.")
		return false
	}
	...
	spinner, _ := utils.Spinner.Start("Downloading DT")

	out, err := os.Create(location)
	if err != nil {
		spinner.Fail("Failed to download DT")
		utils.Fatal(err)
	}
	defer out.Close()
	...
	utils.CheckExistAndDelete(exeOld)
	utils.PrintSuccess("Successfully updated DT to v" + tagName)
	return true
}

func permissionError(err error) {
	utils.PrintInfo("If fatal error is \"Permission denied\", please check read/write permission of dt executable directory.")
	utils.PrintInfo("However, if you used a package manager to install dt, please upgrade by using the same package manager.")
	utils.Fatal(err)
}
