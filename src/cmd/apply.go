...
// Apply .
func Apply(dtVersion string) {
	utils.MigrateConfigFolder()

	backupDTVersion := backupSection.Key("with").MustString("")
	if dtVersion != backupDTVersion {
		utils.PrintInfo(`Preprocessed Spotify data is outdated. Please run "dt restore backup apply" to receive new features and bug fixes`)
		os.Exit(1)
	}
	...
	if preprocSection.Key("expose_apis").MustBool(false) {
		utils.CopyFile(
			filepath.Join(utils.GetJsHelperDir(), "dtWrapper.js"),
	...
	apply.AdditionalOptions(appDestPath, apply.Flag{
		...
		CheckDTUpdate: settingSection.Key("check_dt_update").MustBool(false),
		...
		DTVer:         backupSection.Key("with").MustString(""),
	})
...
func refreshThemeCSS() {
	...
	if err != nil {
		...
		if err := os.WriteFile(
			filepath.Join(appDestPath, "xpui", "dt-config.json"),
...
func CheckStates() {
	...
	if backStat.IsEmpty() {
		if spotStat.IsBackupable() {
			utils.PrintError(`You haven't backed up. Run "dt backup apply"`)
		} else {
			utils.PrintError(`You haven't backed up and Spotify cannot be backed up at this state. Please re-install Spotify then run "dt backup apply"`)
		}
		os.Exit(1)
		...
		if spotStat.IsMixed() {
			...
			utils.PrintInfo(`Please run "dt backup apply"`)
		} else if spotStat.IsStock() {
			utils.PrintInfo(`Spotify client is in stock state`)
			utils.PrintInfo(`Please run "dt backup apply"`)
		} else {
			utils.PrintInfo(`Spotify cannot be backed up at this state. Please re-install Spotify then run "dt backup apply"`)
		}
