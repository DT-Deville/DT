...
func htmlMod(htmlPath string, flags Flag) {
	...
	if flags.DTVer != "" {
		...
		helperHTML += fmt.Sprintf(`<script>
			DT.Config={};
			DT.Config["version"]="%s";
			DT.Config["current_theme"]="%s";
			DT.Config["color_scheme"]="%s";
			DT.Config["extensions"] = [%s];
			DT.Config["custom_apps"] = [%s];
			DT.Config["check_dt_update"]=%v;
		</script>
		`, flags.DTVer, flags.CurrentTheme, flags.ColorScheme, extList, customAppList, flags.CheckDTUpdate)
	}
...
func insertCustomApp(jsPath string, flags Flag) {
	utils.ModifyFile(jsPath, func(content string) string {
		...
		if (len(reactSymbs) < 2) || (len(eleSymbs) == 0) {
			utils.PrintError("Spotify version mismatch with DT. Please report it on our github repository.")
			utils.PrintInfo("DT might have been updated for this version already. Please run `dt update` to check for a new version.")
...
func insertNavLink(str string, appNameArray string) string {
	...
	if libraryXItemMatch != "" {
		str = strings.Replace(
			str,
			libraryXItemMatch,
			fmt.Sprintf("%s,DT._renderNavLinks([%s], false)", libraryXItemMatch, appNameArray),
			1)
	}
	...
	utils.ReplaceOnceWithPriority(&str,
		...
		func(index int, submatches ...string) string {
			switch index {
			case 0, 1:
				return fmt.Sprintf("%s,DT._renderNavLinks([%s], true)]", submatches[1], appNameArray)
			case 2:
				return fmt.Sprintf("%s[%s,DT._renderNavLinks([%s], true)].flat()%s", submatches[1], submatches[2], appNameArray, submatches[3])
			case 3:
				return fmt.Sprintf("%s[%s%s,DT._renderNavLinks([%s], true)].flat()", submatches[1], submatches[2], submatches[3], appNameArray)
			}
			return ""
		},
	)
...
func insertHomeConfig(jsPath string, flags Flag) {
	...
	utils.ModifyFile(jsPath, func(content string) string {
		utils.ReplaceOnce(
			...
			func(submatches ...string) string {
				return fmt.Sprintf("%sDTHomeConfig.arrange(%s)%s", submatches[1], submatches[2], submatches[3])
			})
...
func insertExpFeatures(jsPath string, flags Flag) {
	...
	utils.ModifyFile(jsPath, func(content string) string {
		utils.ReplaceOnce(
			...
			func(submatches ...string) string {
				return fmt.Sprintf("%s%s=DT.expFeatureOverride(%s);%s", submatches[1], submatches[2], submatches[2], submatches[3])
			})
		...
		// 	func(submatches ...string) string {
		// 		return fmt.Sprintf("%s=DT.createInternalMap%s", submatches[1], submatches[2])
		// 	})
		...
		utils.ReplaceOnce(
			...
			func(submatches ...string) string {
				return fmt.Sprintf("%sDT.createInternalMap=%s;%sDT.RemoteConfigResolver=%s", submatches[1], submatches[2], submatches[3], submatches[4])
			})
		...
		utils.ReplaceOnce(
			...
			func(submatches ...string) string {
				return fmt.Sprintf(`%s%s("details",{children: [
					%s("summary",{children: "DT v" + DT.Config.version}),
					%s("li",{children: "Theme: " + DT.Config.current_theme + (DT.Config.color_scheme && " / ") + DT.Config.color_scheme}),
					%s("li",{children: "Extensions: " + DT.Config.extensions.join(", ")}),
					%s("li",{children: "Custom apps: " + DT.Config.custom_apps.join(", ")}),
