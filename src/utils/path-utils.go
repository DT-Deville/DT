...
func MigrateConfigFolder() {
	if runtime.GOOS == "windows" {
		source := filepath.Join(os.Getenv("USERPROFILE"), ".dt")
		if _, err := os.Stat(source); err == nil {
			spinner, _ := Spinner.Start("Migrating config folder")
			destination := GetDTFolder()
...
func MigrateFolders() {
	backupPath := filepath.Join(GetDTFolder(), "Backup")
	extractedPath := filepath.Join(GetDTFolder(), "Extracted")
...
func GetDTFolder() string {
	result, isAvailable := os.LookupEnv("DT_CONFIG")
	...
	if runtime.GOOS == "windows" {
		...
		result = filepath.Join(parent, "dt")
...
func GetStateFolder(name string) string {
	result, isAvailable := os.LookupEnv("DT_STATE")
	defer func() { CheckExistAndCreate(result) }()
...
var userAppsFolder = GetSubFolder(GetDTFolder(), "CustomApps")
var userExtensionsFolder = GetSubFolder(GetDTFolder(), "Extensions")
