...
func init() {
	...
	if isAdmin.Check(bypassAdminCheck) {
		utils.PrintError("DT should NOT be run with administrator or root privileges")
...
func main() {
	...
	if slices.Contains(commands, "upgrade") || slices.Contains(commands, "update") {
		...
		spotStat := spotifystatus.Get(spotifyPath)
		if spotStat.IsBackupable() {
			utils.PrintNote("dt is up-to-date! If you ran this because dt disappeared after Spotify updated, we'll attempt to fix it for you right now.")
...
Run "dt -h" for a list of valid commands.`))
		}
	}
...
func help() {
	...
	log.Println(utils.Bold("USAGE") + "\n" +
		"dt [-q] [-e] [-a] \x1B[4mcommand\033[0m...\n" +
		"dt {-c | --config} | {-v | --version} | {-h | --help}\n\n" +
...
config              1. Print all config fields and values:
                    ...
                    Example usage:
                    dt config color_scheme
                    dt config custom_apps

                    3. Change value of one or multiple config fields.
                    dt config <field> <value> [<field> <value> ...]
...
spotify_launch_flags <string>
    ...
    List of valid flags: https://dt.app/docs/development/spotify-cli-flags

always_enable_devtools <0 | 1>
    Whether Chrome DevTools is enabled when launching/restarting Spotify.

check_dt_update <0 | 1>
    Whether to always check for updates when running DT.
...
expose_apis <0 | 1>
    Leaks some Spotify's API, functions, objects to DT global object that
