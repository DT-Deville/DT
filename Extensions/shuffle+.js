...
(async function shufflePlus() {
	...
	function getConfig() {
		try {
			const parsed = JSON.parse(DT.LocalStorage.get("shufflePlus:settings"));
			if (parsed && typeof parsed === "object") {
				return parsed;
			}
			throw "";
		} catch {
			DT.LocalStorage.set("shufflePlus:settings", "{}");
			return {
				artistMode: "all",
	...
	function saveConfig() {
		DT.LocalStorage.set("shufflePlus:settings", JSON.stringify(CONFIG));
	}
	...
	new DT.Menu.Item("Shuffle+", false, settingsPage, "shuffle").register();

	const { Type } = DT.URI;

	function shouldAddShufflePlus(uri) {
		if (uri.length === 1) {
			const uriObj = DT.URI.fromString(uri[0]);
			switch (uriObj.type) {
				case Type.PLAYLIST:
	...
	function shouldAddShufflePlusLiked(uri) {
		const uriObj = DT.URI.fromString(uri[0]);
		if (DT.Platform.History.location.pathname === "/collection/tracks") {
			switch (uriObj.type) {
				case Type.TRACK:
					return true;
			}
	...
	function shouldAddShufflePlusLocal(uri) {
		const uriObj = DT.URI.fromString(uri[0]);
		if (DT.Platform.History.location.pathname === "/collection/local-files") {
			switch (uriObj.type) {
				case Type.TRACK:
	...
	function renderQueuePlaybarButton() {
		if (!playbarButton) {
			playbarButton = new DT.Playbar.Button(
	...
	async function fetchPlaylistTracks(uri) {
		const res = await DT.Platform.PlaylistAPI.getContents(`spotify:playlist:${uri}`, {
			limit: 9999999,
		});
	...
	async function fetchFolderTracks(uri) {
		const res = await DT.Platform.RootlistAPI.getContents();
		...
		async function fetchNested(folder) {
			...
			for (const i of folder.items) {
				if (i.type === "playlist") {
					const uriObj = DT.URI.fromString(i.uri);
	...
	async function fetchAlbumTracks(uri, includeMetadata = false) {
		const { queryAlbumTracks } = DT.GraphQL.Definitions;
		const { data, errors } = await DT.GraphQL.Request(queryAlbumTracks, {
			uri,
			offset: 0,
			limit: 100,
		});
	...
	async function scanForTracksFromAlbums(res, artistName, type) {
		...
		for (const album of res) {
			...
			DT.showNotification(`${artistFetchTypeCount[type]} / ${res.length} ${type}s`);
	...
	async function fetchArtistTracks(uri) {
		...
		const discography = await DT.GraphQL.Request(queryArtistDiscographyAll, {
			uri,
			offset: 0,
		...
		const overview = await DT.GraphQL.Request(queryArtistOverview, {
			uri,
			locale: DT.Locale.getLocale(),
	...
	async function fetchArtistLikedTracks(uri) {
		const artistRes = await DT.CosmosAsync.get(`sp://core-collection/unstable/@/list/tracks/artist/${uri}?responseFormat=protobufJson`);
	...
	async function fetchArtistTopTenTracks(uri) {
		const { queryArtistOverview } = DT.GraphQL.Definitions;
		const { data, errors } = await DT.GraphQL.Request(queryArtistOverview, {
			uri,
			locale: DT.Locale.getLocale(),
			includePrerelease: false,
		});
	...
	async function fetchLikedTracks() {
		const res = await DT.CosmosAsync.get("sp://core-collection/unstable/@/list/tracks/all?responseFormat=protobufJson");
	...
	async function fetchLocalTracks() {
		const res = await DT.Platform.LocalFilesAPI.getTracks();

		return res.map((track) => track.uri);
	}

	function fetchQueue() {
		const { _queueState } = DT.Platform.PlayerAPI._queue;
	...
	async function fetchCollection(uriObj) {
		...
		const { pathname } = DT.Platform.History.location;
	...
	async function fetchShows(uri) {
		const res = await DT.CosmosAsync.get(`sp://core-show/v1/shows/${uri}?responseFormat=protobufJson`);
	...
	async function Queue(list, context, type) {
		...
		const { _queue, _client } = DT.Platform.PlayerAPI._queue;
		...
		if (context) {
			const { sessionId } = DT.Platform.PlayerAPI.getState();
			DT.Platform.PlayerAPI.updateContext(sessionId, {
				uri: context,
				url: `context://${context}`,
			});
		}
		...
		switch (type) {
			case Type.ARTIST:
				if (CONFIG.artistMode === "topTen") {
					DT.showNotification(`Shuffled Top ${count} Songs`);
					break;
				}
				if (CONFIG.artistMode === "likedSongArtist") {
					DT.showNotification(`Shuffled ${count} Liked Songs`);
					break;
				}
				if (CONFIG.artistMode === "single") {
					DT.showNotification(`Shuffled ${artistFetchTypeCount.single} Singles, Total of ${count} Songs`);
					break;
				}
				if (CONFIG.artistMode === "album") {
					DT.showNotification(`Shuffled ${artistFetchTypeCount.album} Albums, Total of ${count} Songs`);
					break;
				}
				DT.showNotification(`Shuffled ${artistFetchTypeCount.album} Albums, ${artistFetchTypeCount.single} Singles, Total of ${count} Songs`);
				break;
			default:
				DT.showNotification(`Shuffled ${count} Songs`);
		}
	...
	async function fetchAndPlay(rawUri) {
		...
		try {
			if (rawUri === "queue") {
				...
				const uriObj = DT.URI.fromString(rawUri);
				type = uriObj.type;
				...
				if (!list?.length) {
					DT.showNotification("Nothing to play", true);
					return;
				}
			...
			DT.showNotification(String(error), true);
			console.error(error);
		}
	}
})();
