...
(function Bookmark() {
	const { CosmosAsync, Player, LocalStorage, ContextMenu, URI } = DT;
	if (!(CosmosAsync && URI)) {
...
`;
			...
			DT.Tippy(this.querySelectorAll("[data-tippy-content]"), DT.TippyProps);
			if (isPlayable) {
	...
	async function storeThisPage() {
		...
		const context = DT.Platform.History.location.pathname;
		try {
			contextUri = DT.URI.fromString(context);
		} catch (e) {
			DT.showNotification("Cannot bookmark this page", true);
			return;
		}
	...
	function getTrackMeta() {
		...
		const playerState = DT.Player.data;
...
`;
	...
	async function onLinkClick(info) {
		if (info.context?.startsWith("/")) {
			DT.Platform.History.push(info.context);
			return;
		}
		const url = DT.URI.fromString(info.uri).toURLPath(true);
		DT.Platform.History.push(url);
	}
	...
	function onPlayClick(info) {
		...
		DT.Player.playUri(uri, {}, options);
	}

	const fetchAlbum = async (uri) => {
		const { getAlbum } = DT.GraphQL.Definitions;
		const { data } = await DT.GraphQL.Request(getAlbum, {
			uri,
	...
	const fetchArtist = async (uri) => {
		...
		const { data } = await DT.GraphQL.Request(queryArtistOverview, {
			uri,
			locale: DT.Locale.getLocale(),
	...
	const fetchTrack = async (uri, uid, context) => {
		...
		let newContext;
		if (context && uid && DT.URI.isPlaylistV1OrV2(context)) {
			newContext = `${DT.URI.fromString(context).toURLPath(true)}?uid=${uid}`;
		}
		return {
			uri,
			title: res.name,
	...
	const fetchPlaylist = async (uri) => {
		const res = await DT.CosmosAsync.get(`sp://core-playlist/v1/playlist/${uri}/metadata`, {
	...
	new DT.ContextMenu.Item(
		...
		async ([uri], [uid] = [], context = undefined) => {
			...
			let meta;
			switch (type) {
				case DT.URI.Type.TRACK:
					meta = await fetchTrack(uri, uid, context);
					break;
				case DT.URI.Type.ALBUM:
					meta = await fetchAlbum(uri);
					break;
				case DT.URI.Type.ARTIST:
					meta = await fetchArtist(uri);
					break;
				case DT.URI.Type.SHOW:
					meta = await fetchShow(uri);
					break;
				case DT.URI.Type.EPISODE:
					meta = await fetchEpisode(uri);
					break;
				case DT.URI.Type.PLAYLIST:
				case DT.URI.Type.PLAYLIST_V2:
