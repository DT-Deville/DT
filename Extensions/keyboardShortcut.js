...
(function KeyboardShortcut() {
	...
	const binds = {
		...
		"ctrl+q": {
			callback: () =>
				DT.CosmosAsync.post("sp://esperanto/spotify.desktop.lifecycle_esperanto.proto.DesktopLifecycle/Shutdown") &&
				DT.CosmosAsync.post("sp://desktop/v1/shutdown"),
		},
		...
		"shift+h": { callback: () => DT.Platform.History.goBack() },
		"shift+l": { callback: () => DT.Platform.History.goForward() },

		// M to Like/Unlike track
		m: { callback: () => DT.Player.toggleHeart() },

		// Forward Slash to open search page
		"/": { callback: () => DT.Platform.History.replace("/search") },

		// CTRL + Arrow Left Next and CTRL + Arrow Right  Previous Song
		"ctrl+left": { callback: () => DT.Player.back() },
		"ctrl+right": { callback: () => DT.Player.next() },

		// CTRL + Arrow Up Increase Volume CTRL + Arrow Down Decrease Volume
		"ctrl+up": { callback: () => DT.Player.setVolume(DT.Player.getVolume() + 0.05) },
		"ctrl+down": { callback: () => DT.Player.setVolume(DT.Player.getVolume() - 0.05) },
	...
	function findActiveIndex(allItems) {
		...
		const historyURI = DT.Platform.History.location.pathname.replace(/^\//, "spotify:").replace(/\//g, ":");
	...
	function rotateSidebar(direction) {
		const allItems = document.querySelectorAll(
			"#dt-sticky-list .main-yourLibraryX-navLink, .main-yourLibraryX-listItem > div:not(:has([data-skip-in-keyboard-nav])) > div:first-child"
		);
...
.vim-key {
	...
	const mousetrap = new DT.Mousetrap(document);
	...
	/**
	 *
	 * @param {DT.Keyboard.ValidKey} key
	 */
	this.setCancelKey = function (key) {
		mousetrap.bind(DT.Keyboard.KEYS[key], this.deactivate.bind(this));
	};
