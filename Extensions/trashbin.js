...
(function TrashBin() {
	...
	if (!DT.Player.data || !DT.LocalStorage || !skipBackBtn) {
	...
	function createSlider(name, desc, defaultVal, callback) {
		...
		slider.onclick = () => {
			...
			DT.LocalStorage.set(name, state);
	...
	function settingsContent() {
		...
		content.appendChild(
			createButton("Clear ", "Clear all items from trashbin (cannot be reverted).", () => {
				...
				putDataLocal();
				DT.showNotification("Trashbin cleared!");
			})
		);
	}
	...
	function initValue(item, defaultValue) {
		try {
			const value = JSON.parse(DT.LocalStorage.get(item));
	...
	const widget = new DT.Playbar.Widget(
		THROW_TEXT,
		trashbinIcon,
		(self) => {
			const uri = DT.Player.data.item.uri;
			const uriObj = DT.URI.fromString(uri);
			const type = uriObj.type;

			if (!trashSongList[uri]) {
				trashSongList[uri] = true;
				if (shouldSkipCurrentTrack(uri, type)) DT.Player.next();
				DT.showNotification("Song added to trashbin");
			} else {
				delete trashSongList[uri];
				setWidgetState(false);
				DT.showNotification("Song removed from trashbin");
			}
	...
	setWidgetState(
		trashSongList[DT.Player.data.item.uri],
		DT.URI.fromString(DT.Player.data.item.uri).type !== DT.URI.Type.TRACK
	);
	...
	function refreshEventListeners(state) {
		...
		if (state) {
			...
			DT.Player.addEventListener("songchange", watchChange);
			enableWidget && widget.register();
			watchChange();
		} else {
			skipBackBtn.removeEventListener("click", eventListener);
			DT.Player.removeEventListener("songchange", watchChange);
			widget.deregister();
		}
	}
	...
	function watchChange() {
		...
		setWidgetState(isBanned, DT.URI.fromString(data.item.uri).type !== DT.URI.Type.TRACK);
	...
	function shouldSkipCurrentTrack(uri, type) {
		const curTrack = DT.Player.data.item;
		if (type === DT.URI.Type.TRACK) {
			if (uri === curTrack.uri) {
				return true;
			}
		}

		if (type === DT.URI.Type.ARTIST) {
			let count = 1;
	...
	function toggleThrow(uris) {
		...
		const list = type === DT.URI.Type.TRACK ? trashSongList : trashArtistList;

		if (!list[uri]) {
			list[uri] = true;
			if (shouldSkipCurrentTrack(uri, type)) DT.Player.next();
			DT.Player.data?.item.uri === uri && setWidgetState(true);
			DT.showNotification(type === DT.URI.Type.TRACK ? "Song added to trashbin" : "Artist added to trashbin");
		} else {
			delete list[uri];
			DT.Player.data?.item.uri === uri && setWidgetState(false);
			DT.showNotification(type === DT.URI.Type.TRACK ? "Song removed from trashbin" : "Artist removed from trashbin");
		}
	...
	function shouldAddContextMenu(uris) {
		...
		const uri = uris[0];
		const uriObj = DT.URI.fromString(uri);
		if (uriObj.type === DT.URI.Type.TRACK) {
			cntxMenu.name = trashSongList[uri] ? UNTHROW_TEXT : THROW_TEXT;
			return true;
		}

		if (uriObj.type === DT.URI.Type.ARTIST) {
	...
	const cntxMenu = new DT.ContextMenu.Item(THROW_TEXT, toggleThrow, shouldAddContextMenu, trashbinIcon);
	cntxMenu.register();

	function putDataLocal() {
		DT.LocalStorage.set("TrashSongList", JSON.stringify(trashSongList));
		DT.LocalStorage.set("TrashArtistList", JSON.stringify(trashArtistList));
	}
	...
	function copyItems() {
		...
		DT.Platform.ClipboardAPI.copy(JSON.stringify(data));
		DT.showNotification("Copied to clipboard");
	}
	...
	async function exportItems() {
		...
		try {
			const handle = await window.showSaveFilePicker({
				suggestedName: "dt-trashbin.json",
				types: [
					{
						description: "DT trashbin backup",
						accept: {
			...
			DT.showNotification("Backup saved succesfully.");
		} catch {
			DT.showNotification("Failed to save, try copying trashbin contents to clipboard and creating a backup manually.");
		}
	}
		...
		input.onchange = (e) => {
			...
			reader.onload = (e) => {
				try {
					...
					putDataLocal();
					DT.showNotification("File Import Successful!");
				} catch (e) {
					DT.showNotification("File Import Failed!", true);
					console.error(e);
				}
			};
			reader.onerror = () => {
				DT.showNotification("File Read Failed!", true);
