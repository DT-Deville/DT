...
(function FullAppDisplay() {
	if (!DT.Keyboard || !DT.React || !DT.ReactDOM) {
		setTimeout(FullAppDisplay, 200);
		return;
	}

	const { React: react, ReactDOM: reactDOM } = DT;
...
#fad-art {
	...
	const ProgressBar = () => {
		const [progress, setProgress] = useState(DT.Player.getProgress());
		const duration = DT.Platform.PlayerAPI._state.duration;
		...
		useEffect(() => {
			...
			DT.Player.addEventListener("onprogress", update);
			return () => DT.Player.removeEventListener("onprogress", update);
		}, [isDragging]);
		...
		return react.createElement(
			...
			react.createElement("span", { id: "fad-elapsed" }, DT.Player.formatTime(progress)),
			react.createElement(
				"div",
				{
					id: "fad-progress",
			...
			react.createElement("span", { id: "fad-duration" }, DT.Player.formatTime(duration))
		);
	};

	const PlayerControls = () => {
		const [value, setValue] = useState(DT.Player.isPlaying());
		useEffect(() => {
			const update = ({ data }) => setValue(!data.isPaused);
			DT.Player.addEventListener("onplaypause", update);
			return () => DT.Player.removeEventListener("onplaypause", update);
		});
		return react.createElement(
			"div",
			{ id: "fad-controls" },
			react.createElement(ButtonIcon, {
				icon: DT.SVGIcons["skip-back"],
				onClick: DT.Player.back,
			}),
			react.createElement(ButtonIcon, {
				icon: DT.SVGIcons[value ? "pause" : "play"],
				onClick: DT.Player.togglePlay,
			}),
			react.createElement(ButtonIcon, {
				icon: DT.SVGIcons["skip-forward"],
				onClick: DT.Player.next,
			})
		);
	};
	...
	class FAD extends react.Component {
		constructor(props) {
			...
			this.state = {
				...
				releaseDate: "",
				cover: "",
				heart: DT.Player.getHeart(),
			};
			this.currTrackImg = new Image();
			this.nextTrackImg = new Image();
			this.mousetrap = new DT.Mousetrap();
		}

		async getAlbumDate(uri) {
			const { getAlbum } = DT.GraphQL.Definitions;
			const { errors, data } = await DT.GraphQL.Request(getAlbum, {
				uri,
		...
		async fetchInfo() {
			const meta = DT.Player.data.item.metadata;
		...
		componentDidMount() {
			...
			DT.Player.addEventListener("songchange", this.updateInfo);
			this.updateInfo();
			...
			DT.Platform.PlayerAPI._events.addListener("queue_update", this.onQueueChange);
			this.mousetrap.bind("esc", deactivate);
			window.dispatchEvent(new Event("fad-request"));
		}

		componentWillUnmount() {
			DT.Player.removeEventListener("songchange", this.updateInfo);
			DT.Platform.PlayerAPI._events.removeListener("queue_update", this.onQueueChange);
								...
								react.createElement(
									...
									react.createElement(
										...
										react.createElement(DisplayIcon, {
											icon: DT.SVGIcons[this.state.heart ? "heart-active" : "heart"],
											size: 50,
										})
									)
								),
	...
	function deactivate() {
		...
		if (lastApp && lastApp !== "/lyrics-plus") {
			DT.Platform.History.push(lastApp);
		}
	}
	...
	function checkLyricsPlus() {
		return DT.Config?.custom_apps?.includes("lyrics-plus") || !!document.querySelector("a[href='/lyrics-plus']");
	}

	function requestLyricsPlus() {
		if (CONFIG.lyricsPlus && checkLyricsPlus()) {
			lastApp = DT.Platform.History.location.pathname;
			if (lastApp !== "/lyrics-plus") {
				DT.Platform.History.push("/lyrics-plus");
			}
		}
		window.dispatchEvent(new Event("fad-request"));
	}
	...
	function getConfig() {
		try {
			const parsed = JSON.parse(DT.LocalStorage.get("full-app-display-config") || "{}");
			if (parsed && typeof parsed === "object") {
				return parsed;
			}
			throw "";
		} catch {
			DT.LocalStorage.set("full-app-display-config", "{}");
			return {};
		}
	}

	function saveConfig() {
		DT.LocalStorage.set("full-app-display-config", JSON.stringify(CONFIG));
	}
...
`,
	...
	new DT.Topbar.Button(
		"Full App Display",
		`<svg role="img" height="16" width="16" viewBox="0 0 16 16" fill="currentColor">${DT.SVGIcons.projector}</svg>`,
		activate
	);

	DT.Mousetrap.bind("f11", toggleFad);
})();
