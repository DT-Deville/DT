...
`;
	...
	function saveState() {
		...
		if (!uri) return;
		DT.LocalStorage.set(`loopyLoop:${uri}`, JSON.stringify({ start, end, skipZones }));
	}
	...
	function loadState() {
		...
		if (!uri) return;
		try {
			const saved = DT.LocalStorage.get(`loopyLoop:${uri}`);
			if (saved) {
	...
	function applyMoveAdjustment(deltaSeconds) {
		const durationMs = DT.Player.getDuration();
	...
	DT.Player.addEventListener("onprogress", (event) => {
		...
		const durationMs = DT.Player.getDuration() || 0;
		...
		if (prevProgressPercent > threeSecFrac && percent < nearZeroFrac) {
			if (prevPressedAt > 0 && ts - prevPressedAt < 1500) {
				...
				prevPressedAt = ts;
				prevProgressPercent = percent;
				if (start !== null) DT.Player.seek(start);
				return;
			}
		}
		prevProgressPercent = percent;
		...
		if (end !== null && percent >= end) {
			// DT.Player.getRepeat(): 0 = off, 1 = repeat context, 2 = repeat track
			if (DT.Player.getRepeat() === 2) {
				if (ts - lastEndLoopSeek > 500) {
					lastEndLoopSeek = ts;
					DT.Player.seek(start ?? 0);
				}
			} else if (ts - lastNextCall > 2000) {
				lastNextCall = ts;
				seekStartPendingUri = DT.Player.data?.item?.uri ?? null;
				DT.Player.next();
			}
			return;
		}
	...
	DT.Player.addEventListener("songchange", () => {
		...
		if (DT.Player.data?.item?.uri !== seekStartPendingUri) seekStartPendingUri = null;
		loadState();
		drawOnBar();
		drawSkipMarkers();
	...
	const startBtn = createMenuItem("Set song start", () => {
		if (end !== null && mouseOnBarPercent >= end) {
			DT.showNotification("Song start must be before song end");
			return;
		}
	...
	const endBtn = createMenuItem("Set song end", () => {
		if (start !== null && mouseOnBarPercent <= start) {
			DT.showNotification("Song end must be after song start");
			return;
		}
	...
	const skipEndBtn = createMenuItem("Set section skip end", () => {
		if (pendingSkipStart === null) {
			DT.showNotification("No section skip start selected!");
			return;
		}
		...
		if (e > s) {
			if (skipZones.length < 10) {
				...
				DT.showNotification("Maximum 10 skip zones reached");
			}
		}
		pendingSkipStart = null;
	});
	...
	try {
		...
		const toolbarBtn = new DT.Playbar.Button("Loopy Loop", markerIcon, () => {
			mouseOnBarPercent = DT.Player.getProgressPercent();
