...
/** @type {React} */
const react = DT.React;
const { useState, useEffect, useCallback, useMemo, useRef } = react;
/** @type {import("react").ReactDOM} */
const spotifyVersion = DT.Platform.version;
...
class LyricsContainer extends react.Component {
	...
	async fetchColors(uri) {
		...
		try {
			try {
				const { fetchExtractedColorForTrackEntity } = DT.GraphQL.Definitions;
				const { data } = await DT.GraphQL.Request(fetchExtractedColorForTrackEntity, { uri });
				const { hex } = data.trackUnion.albumOfTrack.coverArt.extractedColors.colorDark;
				vibrant = Number.parseInt(hex.replace("#", ""), 16);
			} catch {
				const colors = await DT.CosmosAsync.get(`https://spclient.wg.spotify.com/colorextractor/v1/extract-presets?uri=${uri}&format=json`);
	...
	async fetchTempo(uri) {
		const audio = await DT.CosmosAsync.get(
	...
	async refreshMusixmatchTranslation() {
		...
		DT.showNotification(MUSIXMATCH_TRANSLATION_FETCH_MESSAGE, false, 1000);
		...
		try {
			...
			console.error(error);
			if (isLatestRequest()) {
				DT.showNotification(MUSIXMATCH_TRANSLATION_FETCH_FAILED_MESSAGE, true, 3000);
				if (CACHE[currentUri]) {
					CACHE[currentUri].musixmatchTranslation = null;
					CACHE[currentUri].musixmatchTranslationLanguage = null;
				}
	...
	async translateLyrics(language, lyrics, targetConvert) {
		...
		DT.showNotification("Converting...", false, 1000);
		if (!this.translator) {
		...
		try {
			if (language === "ja") {
				...
				if (targetConvert === "cn") {
					DT.showNotification("No conversion is needed", false, 1000);
					return lyrics;
				}
			...
			DT.showNotification("Converting...", false, 0);
			return res;
		} catch (error) {
			DT.showNotification("Convert Error!", true);
			console.error(error);
		}
	}

	resetDelay() {
		CONFIG.visual.delay = Number(localStorage.getItem(`lyrics-delay:${DT.Player.data.item.uri}`)) || 0;
	}
	...
	processLyricsFromFile(event) {
		...
		if (file[0].size > 1024 * 1024) {
			DT.showNotification("File too large", true);
			return;
		}
		...
		reader.onload = (e) => {
			try {
				...
				if (!parsedKeys.length) {
					DT.showNotification("Nothing to load", true);
					return;
				}
				...
				DT.showNotification(`Loaded ${parsedKeys.join(", ")} lyrics from file`);
			} catch (e) {
				console.error(e);
				DT.showNotification("Failed to load lyrics", true);
			}
		};

		reader.onerror = (e) => {
			console.error(e);
			DT.showNotification("Failed to read file", true);
		};
	...
	initMoustrap() {
		if (!this.mousetrap && DT.Mousetrap) {
	...
	componentDidMount() {
		...
		if (DT.Player?.data?.item) {
			this.state.explicitMode = this.state.lockMode;
			this.currentTrackUri = DT.Player.data.item.uri;
			this.fetchLyrics(DT.Player.data.item, this.state.explicitMode);
		}
		...
		reloadLyrics = () => {
			...
			this.updateVisualOnConfigChange();
			this.forceUpdate();
			this.fetchLyrics(DT.Player.data.item, this.state.explicitMode, true);
		};
		...
		this.configButton = new DT.Menu.Item("Lyrics Plus config", false, openConfig, "lyrics");
	...
	render() {
					...
					react.createElement(
						...
						react.createElement("svg", {
							...
							dangerouslySetInnerHTML: {
								__html: DT.SVGIcons[this.state.isCached ? "downloaded" : "download"],
							},
						})
					)
				),
				...
				react.createElement(TopBarContent, {
					...
					switchCallback: (label) => {
						...
						if (mode !== this.state.mode) {
							...
							const info = this.infoFromTrack(DT.Player.data.item);
							if (info?.uri && CACHE[info?.uri]) {
								CACHE[info.uri].mode = mode;
							}

							this.setState({ explicitMode: mode });
							this.state.provider !== "local" && this.fetchLyrics(DT.Player.data.item, mode);
						}
					},
					lockCallback: (label) => {
						...
						this.fetchLyrics(DT.Player.data.item, mode);
						CONFIG.locked = mode;
		...
		if (this.state.isFullscreen) return DT.ReactDOM.createPortal(out, this.fullscreenContainer);
		if (fadLyricsContainer) return DT.ReactDOM.createPortal(out, fadLyricsContainer);
		return out;
	}
}
