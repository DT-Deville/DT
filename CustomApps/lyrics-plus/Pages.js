const CreditFooter = react.memo(({ provider, copyright }) => {
	...
	const credit = [DT.Locale.get("web-player.lyrics.providedBy", provider)];
	if (copyright) {
...
const SyncedLyricsPage = react.memo(({ lyrics = [], provider, copyright, isKara }) => {
	...
	useTrackPosition(() => {
		const newPos = DT.Player.getProgress();
						...
						: react.createElement(
								"p",
								{
									onContextMenu: (event) => {
										...
										DT.Platform.ClipboardAPI.copy(Utils.convertParsedToLRC(lyrics, belowMode).original)
											.then(() => DT.showNotification("Lyrics copied to clipboard"))
											.catch(() => DT.showNotification("Failed to copy lyrics to clipboard"));
									},
								},
					...
					belowMode &&
						react.createElement(
							"p",
							{
								...
								onContextMenu: (event) => {
									event.preventDefault();
									DT.Platform.ClipboardAPI.copy(Utils.convertParsedToLRC(lyrics, belowMode).conver)
										.then(() => DT.showNotification("Translated lyrics copied to clipboard"))
										.catch(() => DT.showNotification("Failed to copy translated lyrics to clipboard"));
								},
							},
							text
						)
...
class SearchBar extends react.Component {
	...
	componentDidMount() {
		...
		this.toggleCallback = () => {
			if (!(DT.Platform.History.location.pathname === "/lyrics-plus" && this.container)) return;
		...
		DT.Mousetrap().bind("mod+shift+f", this.toggleCallback);
		DT.Mousetrap(this.container).bind("mod+shift+f", this.toggleCallback);
		DT.Mousetrap(this.container).bind("enter", this.loopThroughCallback);
		DT.Mousetrap(this.container).bind("shift+enter", this.loopThroughCallback);
		DT.Mousetrap(this.container).bind("esc", this.unFocusCallback);
	}

	componentWillUnmount() {
		DT.Mousetrap().unbind("mod+shift+f", this.toggleCallback);
		DT.Mousetrap(this.container).unbind("mod+shift+f", this.toggleCallback);
		DT.Mousetrap(this.container).unbind("enter", this.loopThroughCallback);
		DT.Mousetrap(this.container).unbind("shift+enter", this.loopThroughCallback);
		DT.Mousetrap(this.container).unbind("esc", this.unFocusCallback);
	}
...
const SyncedExpandedLyricsPage = react.memo(({ lyrics, provider, copyright, isKara }) => {
	const [position, setPosition] = useState(() => DT.Player.getProgress() + CONFIG.visual["global-delay"] + CONFIG.visual.delay);
	const activeLineRef = useRef(null);
	const pageRef = useRef(null);

	useTrackPosition(() => {
		if (!DT.Player.data.is_paused) {
			setPosition(DT.Player.getProgress() + CONFIG.visual["global-delay"] + CONFIG.visual.delay);
		}
	});
			...
			return react.createElement(
				...
				react.createElement(
					"p",
					{
						onContextMenu: (event) => {
							event.preventDefault();
							DT.Platform.ClipboardAPI.copy(Utils.convertParsedToUnsynced(lyrics, belowMode).original)
								.then(() => DT.showNotification("Lyrics copied to clipboard"))
								.catch(() => DT.showNotification("Failed to copy lyrics to clipboard"));
						},
					},
				...
				belowMode &&
					react.createElement(
						"p",
						{
							...
							onContextMenu: (event) => {
								event.preventDefault();
								DT.Platform.ClipboardAPI.copy(Utils.convertParsedToUnsynced(lyrics, belowMode).conver)
									.then(() => DT.showNotification("Translated lyrics copied to clipboard"))
									.catch(() => DT.showNotification("Failed to copy translated lyrics to clipboard"));
							},
						},
		...
		const lyricsEl1 = react.createElement(
			...
			react.createElement("div", {
				...
				onContextMenu: (event) => {
					...
					DT.Platform.ClipboardAPI.copy(copylyrics)
						.then(() => DT.showNotification("Lyrics copied to clipboard"))
						.catch(() => DT.showNotification("Failed to copy lyrics to clipboard"));
				},
			})
