...
let dismissed;
try {
	dismissed = JSON.parse(DT.LocalStorage.get("new-releases:dismissed"));
...
class Grid extends react.Component {
	...
	removeCards(id, type) {
		switch (type) {
			case "reset":
				DT.showNotification("Reset dismissed releases");
				dismissed = [];
				break;
			case "undo":
				if (!dismissed[0]) DT.showNotification("Nothing to undo", true);
				else DT.showNotification("Undone dismissal");
		...
		DT.LocalStorage.set("new-releases:dismissed", JSON.stringify(dismissed));
	...
	async componentDidMount() {
		...
		this.configButton = new DT.Menu.Item(
			"New Releases config",
			false,
			openConfig,
	...
	render() {
		...
		const version = DT.Platform.version.split(".").map((i) => Number.parseInt(i));
		...
		return react.createElement(
			...
			react.createElement(
				...
				react.createElement("h1", null, DT.Locale.get("new_releases")),
				react.createElement(
					"div",
					{
					...
					react.createElement(ButtonText, {
						text: DT.Locale.get("playlist.extender.refresh"),
...
async function getArtistList() {
	...
	const artists = await DT.Platform.LibraryAPI.getContents(config);
	count(true);
	return artists.items ?? [];
}

async function getArtistEverything(artist) {
	const { data, errors } = await DT.GraphQL.Request(
		{
	...
	const items = [];
	const types = [
		[CONFIG.album, releases.filter((r) => r.type === "ALBUM"), DT.Locale.get("album")],
		...
		// [CONFIG["appears-on"], releases.appears_on?.releases, DT.Locale.get("artist.appears-on")],
		[CONFIG.compilations, releases.filter((r) => r.type === "COMPILATION"), DT.Locale.get("compilation")],
		[
			CONFIG["single-ep"],
			releases.filter((r) => r.type === "SINGLE" || r.type === "EP"),
			`${DT.Locale.get("single")}/${DT.Locale.get("ep")}`,
		],
	];
...
async function getPodcastList() {
	const body = await DT.Platform.LibraryAPI.getShows({ limit: 50000 });
	return body.items ?? [];
}

async function getPodcastRelease(uri) {
	const body = await DT.Platform.ShowAPI.getContents(uri, { limit: 50000 });
	return body.items;
}
...
async function fetchTracks() {
	...
	DT.showNotification(`Fetching releases from ${artistList.length} artists`);
...
async function fetchPodcasts() {
	const items = [];
	const itemTypeStr = DT.Locale.get("card.tag.episode");
