function DraggableComponent({ uri, title, children }) {
	const dragHandler = DT.ReactHook.DragHandler?.([uri], title);
	return dragHandler
...
class Card extends react.Component {
	constructor(props) {
		...
		const uriType = DT.URI.fromString(this.uri)?.type;
		switch (uriType) {
			case DT.URI.Type.ALBUM:
			case DT.URI.Type.TRACK:
				this.menuType = DT.ReactComponent.AlbumMenu;
				break;
		}
	...
	play(event) {
		DT.Player.playUri(this.uri, this.context);
	...
	closeButtonClicked(event) {
		...
		DT.Snackbar.enqueueCustomSnackbar
			? DT.Snackbar.enqueueCustomSnackbar("dismissed-release", {
					keyPrefix: "dismissed-release",
					children: DT.ReactComponent.Snackbar.wrapper({
						children: DT.ReactComponent.Snackbar.simpleLayout({
							leading: DT.ReactComponent.Snackbar.styledImage({
								src: this.props.imageURL,
								imageHeight: "24px",
								imageWidth: "24px",
							}),
							center: DT.React.createElement("div", {
								dangerouslySetInnerHTML: {
									__html: `Dismissed <b>${this.title}</b>.`,
								},
							}),
							trailing: DT.ReactComponent.Snackbar.ctaText({
								ctaText: "Undo",
			...
			: DT.showNotification(`Dismissed <b>${this.title}</b> from <br>${this.artist.name}</b>`);
	}
	...
	render() {
		...
		if (this.visual.count && this.trackCount) {
			detail.push(DT.Locale.get("tracklist-header.songs-counter", this.trackCount));
		}

		return react.createElement(
			DT.ReactComponent.RightClickMenu || "div",
			{
