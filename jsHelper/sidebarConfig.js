(function SidebarConfig() {
	...
	function writeStorage() {
		...
		return localStorage.setItem("dt-sidebar-config:ylx", JSON.stringify(array));
	}

	const container = document.createElement("div");
	container.id = "dt-sidebar-config";
...
#dt-sidebar-config button:disabled {
...
`;
	...
	function injectInteraction() {
		...
		YLXSidebarState = DT.Platform.LocalStorageAPI.getItem("ylx-sidebar-state");
	...
	function removeInteraction() {
		...
		if (YLXSidebarState === 1) document.querySelector(".main-yourLibraryX-collapseButton > button")?.click();
			...
			document.documentElement.style.setProperty(
				"--nav-bar-width",
				`${DT.Platform.LocalStorageAPI.getItem(
	...
	(async () => {
		...
		while (!DT.Snackbar?.enqueueCustomSnackbar) {
			await new Promise((resolve) => setTimeout(resolve, 10));
		}

		if (document.querySelector(".Root__globalNav")) {
			const content = DT.React.createElement("div", {
				dangerouslySetInnerHTML: {
					__html:
						"Sidebar config is not supported when Global Navbar is enabled.<br>In your terminal, please run <code>dt config sidebar_config 0</code> command and then re-apply dt with <code>dt apply</code>.",
				},
				style: {
					"text-size": "12px",
				},
			...
			DT.Snackbar?.enqueueCustomSnackbar("sidebar-config", {
				keyPrefix: "sidebar-config",
				autoHideDuration: 7500,
				children: DT.ReactComponent.Snackbar.wrapper({
					children: DT.React.createElement(DT.ReactComponent.Snackbar.simpleLayout, {
						center: content,
						children: content,
						dragMetadata: {},
					}),
	...
	function InitSidebarXConfig() {
		...
		appItems.id = "dt-sticky-list";
		// SHOW container
		list = document.createElement("ul");
		list.id = "dt-show-list";
		// HIDDEN container
		hiddenList = document.createElement("ul");
		hiddenList.id = "dt-hidden-list";
		hiddenList.classList.add("hidden-visually");
		const playlistList = libraryItems.querySelector("ul");
		playlistList.id = "dt-playlist-list";
		...
		let storage = [];
		try {
			storage = JSON.parse(localStorage.getItem("dt-sidebar-config:ylx"));
		...
		const observer = new MutationObserver((mutations) => {
			for (const mutation of mutations) {
				if (mutation.type === "childList" && mutation.addedNodes.length) {
					mutation.addedNodes[0].id = "dt-playlist-list";
				}
			}
		});
...
.Root__nav-bar :not(.main-yourLibraryX-entryPoints) > #dt-show-list >* {
padding: 0 24px 0 8px;
}
.main-yourLibraryX-entryPoints #dt-show-list,
.main-yourLibraryX-entryPoints #dt-hidden-list {
padding: 0 12px;
}
`;
