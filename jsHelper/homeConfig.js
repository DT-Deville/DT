...
(async () => {
	...
	DTHomeConfig.arrange = (sections) => {
		mounted = true;
		if (list) {
			return list;
		}
		const stickList = (localStorage.getItem("dt-home-config:stick") || "").split(",");
		const lowList = (localStorage.getItem("dt-home-config:low") || "").split(",");
		const stickSections = [];
		const lowSections = [];
...
#dt-home-config button:disabled {
    color: var(--spice-button-disabled);
}
`;

	const container = document.createElement("div");
	container.id = "dt-home-config";
	...
	DTHomeConfig.menu = new DT.Menu.Item(
		"Home config",
		false,
		(self) => {
		...
		DT.SVGIcons["grid-view"]
	);

	DTHomeConfig.addToMenu = () => {
		DTHomeConfig.menu.register();
	};
	DTHomeConfig.removeMenu = () => {
		DTHomeConfig.menu.setState(false);
		DTHomeConfig.menu.deregister();
	};

	await new Promise((res) => DT.Events.platformLoaded.on(res));
	// Init
	if (DT.Platform.History.location.pathname === "/") {
		DTHomeConfig.addToMenu();
	}

	DT.Platform.History.listen(({ pathname }) => {
		if (pathname === "/") {
			DTHomeConfig.addToMenu();
		} else {
			DTHomeConfig.removeMenu();
