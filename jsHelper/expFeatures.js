(async () => {
	...
	try {
		overrideList = JSON.parse(localStorage.getItem("dt-exp-features"));
	...
	DT.expFeatureOverride = (feature) => {
		...
		localStorage.setItem("dt-exp-features", JSON.stringify(overrideList));
		return feature;
	};

	const content = document.createElement("div");
	content.classList.add("dt-exp-features");
	const style = document.createElement("style");
	style.innerHTML = `
.dt-exp-features .col {
    padding: 0;
}
.dt-exp-features .setting-row::after {
    content: "";
    display: table;
    clear: both;
}
.dt-exp-features .setting-row {
    display: flex;
    padding: 10px 0;
    align-items: center;
}
.dt-exp-features .setting-row .col.description {
    float: left;
    padding-right: 15px;
    width: 100%;
}
.dt-exp-features .setting-row .col.action {
    float: right;
    text-align: right;
}
.dt-exp-features .setting-row .col.action .dropdown {
	width: max-content;
}
.dt-exp-features button.switch {
    align-items: center;
    border: 0px;
    border-radius: 50%;
...
.dt-exp-features button.switch.disabled,
.dt-exp-features button.switch[disabled] {
    color: rgba(var(--spice-rgb-text), .3);
}
.dt-exp-features button.reset {
	font-weight: 700;
	font-size: medium;
...
.dt-exp-features button.reset:hover {
	transform: scale(1.04);
	border-color: var(--spice-text);
}
.dt-exp-features .search-container {
    width: 100%;
}
.dt-exp-features .setting-row#search .col.action {
    position: relative;
    width: 100%;
}
.dt-exp-features .setting-row#search svg {
    position: absolute;
    margin: 12px;
}
.dt-exp-features input.search {
    border-style: solid;
	...
	(function waitForRemoteConfigResolver() {
		...
		if (!hooksPatched || (!DT.RemoteConfigResolver && !DT.Platform?.RemoteConfigDebugAPI && !DT.Platform?.RemoteConfiguration)) {
			setTimeout(waitForRemoteConfigResolver, 500);
			return;
		}

		let remoteConfiguration = DT.RemoteConfigResolver?.value.remoteConfiguration || DT.Platform?.RemoteConfiguration;
		const setOverrides = async (overrides) => {
			if (DT.Platform?.RemoteConfigDebugAPI) {
				for (const [name, value] of Object.entries(overrides)) {
					const feature = overrideList[name];
					const type = feature.values ? "enum" : typeof value === "number" ? "number" : "boolean";
					await DT.Platform.RemoteConfigDebugAPI.setOverride(
						{
							source: "web",
							type,
							name,
			...
			} else if (DT.RemoteConfigResolver?.value?.setOverrides) {
				DT.RemoteConfigResolver.value.setOverrides(DT.createInternalMap?.(overrides));
			}
		};

		(async function waitForResolver() {
			if (!DT.RemoteConfigResolver && !DT.Platform?.RemoteConfigDebugAPI) {
				isFallback = true;
			...
			notice.remove();
			remoteConfiguration =
				DT?.RemoteConfigResolver?.value.remoteConfiguration ?? (await DT.Platform?.RemoteConfigDebugAPI.getProperties());
		})();

		for (const key of Object.keys(overrideList)) {
			if (newFeatures.includes(key)) continue;
			delete overrideList[key];
			console.warn(`[dt-exp-features] Removed ${key} from override list`);
			localStorage.setItem("dt-exp-features", JSON.stringify(overrideList));
		}
...
<input type="text" class="search" placeholder="Search for a feature">
		...
		resetBtn.onclick = () => {
			localStorage.removeItem("dt-exp-features");
