window.DT = {
	Player: {
		addEventListener: (type, callback) => {
			if (!(type in DT.Player.eventListeners)) {
				DT.Player.eventListeners[type] = [];
			}
			DT.Player.eventListeners[type].push(callback);
		},
		dispatchEvent: (event) => {
			if (!(event.type in DT.Player.eventListeners)) {
				return true;
			}
			const stack = DT.Player.eventListeners[event.type];
		...
		eventListeners: {},
		seek: (p) => {
			const duration = p <= 1 ? Math.round(p * DT.Player.origin._state.duration) : p;
			DT.Player.origin.seekTo(duration);
		},
		getProgress: () =>
			(DT.Player.origin._state.isPaused ? 0 : Date.now() - DT.Player.origin._state.timestamp) +
			DT.Player.origin._state.positionAsOfTimestamp,
		getProgressPercent: () => DT.Player.getProgress() / DT.Player.origin._state.duration,
		getDuration: () => DT.Player.origin._state.duration,
		setVolume: (v) => {
			DT.Platform.PlaybackAPI.setVolume(v);
		},
		increaseVolume: () => {
			DT.Platform.PlaybackAPI.raiseVolume();
		},
		decreaseVolume: () => {
			DT.Platform.PlaybackAPI.lowerVolume();
		},
		getVolume: () => DT.Platform.PlaybackAPI._volume,
		next: () => {
			DT.Player.origin.skipToNext();
		},
		back: () => {
			DT.Player.origin.skipToPrevious();
		},
		togglePlay: () => {
			DT.Player.isPlaying() ? DT.Player.pause() : DT.Player.play();
		},
		isPlaying: () => !DT.Player.origin._state.isPaused,
		toggleShuffle: () => {
			DT.Player.origin.setShuffle(!DT.Player.origin._state.shuffle);
		},
		getShuffle: () => DT.Player.origin._state.shuffle,
		setShuffle: (b) => {
			DT.Player.origin.setShuffle(b);
		},
		toggleRepeat: () => {
			DT.Player.origin.setRepeat((DT.Player.origin._state.repeat + 1) % 3);
		},
		getRepeat: () => DT.Player.origin._state.repeat,
		setRepeat: (r) => {
			DT.Player.origin.setRepeat(r);
		},
		getMute: () => DT.Player.getVolume() === 0,
		toggleMute: () => {
			DT.Player.setMute(!DT.Player.getMute());
		},
		setMute: (b) => {
			if (b !== DT.Player.getMute()) {
		...
		getHeart: () => DT.Player.origin._state.item.metadata["collection.in_collection"] === "true",
		pause: () => {
			DT.Player.origin.pause();
		},
		play: () => {
			DT.Player.origin.resume();
		},
		playUri: async (uri, context = {}, options = {}) => {
			return await DT.Player.origin.play({ uri: uri }, context, options);
		},
		removeEventListener: (type, callback) => {
			if (!(type in DT.Player.eventListeners)) return;
			const stack = DT.Player.eventListeners[type];
			for (let i = 0; i < stack.length; i++) {
				if (stack[i] === callback) {
					stack.splice(i, 1);
					return;
		...
		skipBack: (amount = 15e3) => {
			DT.Player.origin.seekBackward(amount);
		},
		skipForward: (amount = 15e3) => {
			DT.Player.origin.seekForward(amount);
		},
		setHeart: (b) => {
			const uris = [DT.Player.origin._state.item.uri];
			if (b) {
				DT.Platform.LibraryAPI.add({ uris });
			} else {
				DT.Platform.LibraryAPI.remove({ uris });
			}
		},
		toggleHeart: () => {
			DT.Player.setHeart(!DT.Player.getHeart());
		},
	},
	test: () => {
		function checkObject(object) {
			...
			for (const method of methods) {
				if (objectToCheck[method] === undefined || objectToCheck[method] === null) {
					console.error(`${name}.${method} is not available. Please open an issue in the DT repository to inform us about it.`);
					count--;
				}
			}
		...
		const objectsToCheck = new Set([
			...
			{
				objectToCheck: DT.ReactComponent,
				name: "DT.ReactComponent",
				methods: new Set([
					"RightClickMenu",
					"ContextMenu",
					"Menu",
			...
			{
				objectToCheck: DT.ReactComponent.Cards,
				name: "DT.ReactComponent.Cards",
				methods: new Set([
					"Default",
					"Hero",
					"CardImage",
...
(function waitForPlatform() {
	...
	for (const key of Object.keys(_platform)) {
		if (key.startsWith("get") && typeof _platform[key] === "function") {
			DT.Platform[key.slice(3)] = _platform[key]();
		} else {
			DT.Platform[key] = _platform[key];
		}
	}
})();

(function addMissingPlatformAPIs() {
	if (!DT.Platform?.version && !DT.Platform?.Registry) {
		setTimeout(addMissingPlatformAPIs, 50);
		return;
	}
	const os = DT.Platform.operatingSystem;
	const version = DT.Platform.version.split(".").map((i) => Number.parseInt(i, 10));
	if (version[0] === 1 && version[1] === 2 && version[2] < 38) return;

	for (const [key, _] of DT.Platform.Registry._map.entries()) {
		...
		if (Object.hasOwn(DT.Platform, symbolName)) continue;
		try {
			const resolvedAPI = DT.Platform.Registry.resolve(key);
			DT.Platform[symbolName] = resolvedAPI;

			console.debug(`[dtWrapper] Resolved PlatformAPI from Registry: ${symbolName}`);
		} catch (err) {
			console.error(`[dtWrapper] Error resolving PlatformAPI from Registry: ${symbolName}`, err);
		}
	}
})();
...
(async function addProxyCosmos() {
	if (!DT.Player.origin?._cosmos && !DT.Platform?.Registry) {
		setTimeout(addProxyCosmos, 50);
		return;
	}

	const _cosmos = DT.Player.origin?._cosmos ?? DT.Platform?.Registry.resolve(Symbol.for("Cosmos"));
	...
	const handler = {
		get: (target, prop, receiver) => {
			...
			return async function (url, body) {
				...
				const corsProxyURLTemplate = window.localStorage.getItem("dt:corsProxyTemplate") ?? "https://cors-proxy.dt.app/{url}";
				...
				if (shouldUseCORSProxy) {
					...
					try {
						...
						console.error("[dtWrapper] Invalid CORS Proxy URL template");
					}
				}

				const Authorization = `Bearer ${DT.Platform.AuthorizationAPI.getState().token.accessToken}`;
				let injectedHeaders = {};
				if (isWebAPI) injectedHeaders = { Authorization };
				if (isSpClientAPI) {
					injectedHeaders = {
						Authorization,
						"Spotify-App-Version": DT.Platform.version,
						"App-Platform": DT.Platform.PlatformData.app_platform,
					};
				}
	...
	while (!DT.Player.origin) await new Promise((r) => setTimeout(r, 50));
	DT.Player.origin._cosmos = new Proxy(_cosmos, handler);
	Object.defineProperty(DT, "CosmosAsync", {
		get: () => {
			return DT.Player.origin?._cosmos;
		},
	});
})();
...
(async function hotloadWebpackModules() {
	...
	console.log("[dtWrapper] Waiting for required webpack modules to load");
	...
	DT.React = cache.find((m) => m?.useMemo);

	while (!webpackDidCallback) {
		await new Promise((r) => setTimeout(r, 100));
	}
	console.log("[dtWrapper] All required webpack modules loaded");
	chunks = Object.entries(require.m);
	cache = Object.keys(require.m).map((id) => require(id));
	DT.Events.platformLoaded.fire();
	...
	const _ScrollableContainer = (() => {
		...
		function injectStyles() {
			...
			style.className = "dt-scrollable-container";
			style.textContent = `
.dt-sc-contentArea { overflow: hidden; position: relative; }
.dt-sc-scroller { display: flex; align-items: center; overflow-x: auto; scrollbar-width: none; white-space: nowrap; width: 100%; -ms-overflow-style: none; overscroll-behavior-x: contain; will-change: transform; }
@media (prefers-reduced-motion: no-preference) { .dt-sc-scroller { scroll-behavior: smooth; } }
.dt-sc-scroller::-webkit-scrollbar { display: none; }
.dt-sc-scroller.dt-sc-snap { scroll-snap-type: inline mandatory; }
.dt-sc-scroller.dt-sc-snap .dt-sc-snapCenter [data-carousel-item] { scroll-snap-align: center; }
.dt-sc-scroller.dt-sc-snap .dt-sc-snapStart [data-carousel-item] { scroll-snap-align: start; }
.dt-sc-scroller.dt-sc-wheelEnabled { overscroll-behavior: contain; }
.dt-sc-scroller.dt-sc-maskGradient { --sc-start-color: #000; --sc-end-color: #000; -webkit-mask-composite: source-in, xor; mask-composite: intersect; -webkit-mask-image: linear-gradient(90deg, var(--sc-start-color) 0, #000 120px), linear-gradient(90deg, #000 calc(100% - 120px), var(--sc-end-color) 100%); mask-image: linear-gradient(90deg, var(--sc-start-color) 0, #000 120px), linear-gradient(90deg, #000 calc(100% - 120px), var(--sc-end-color) 100%); -webkit-mask-size: 100% 100%; mask-size: 100% 100%; }
.dt-sc-scroller.dt-sc-maskGradient.dt-sc-maskStart { --sc-start-color: transparent; }
.dt-sc-scroller.dt-sc-maskGradient.dt-sc-maskEnd { --sc-end-color: transparent; }
.dt-sc-linearGradient::before, .dt-sc-linearGradient::after { bottom: 0; content: ""; height: 100%; opacity: 0; pointer-events: none; position: absolute; top: 0; transition: opacity .15s ease-out; width: 120px; z-index: 2; }
.dt-sc-linearGradient::before { background: linear-gradient(90deg, var(--carousel-start-chevron-gradient, var(--spice-main)) 0, transparent 100%); inset-inline-start: 0; }
.dt-sc-linearGradient::after { background: linear-gradient(-90deg, var(--carousel-end-chevron-gradient, var(--spice-main)) 0, transparent 100%); inset-inline-end: 0; }
.dt-sc-linearGradient.dt-sc-lgStart::before { opacity: 1; }
.dt-sc-linearGradient.dt-sc-lgEnd::after { opacity: 1; }
.dt-sc-carousel { bottom: 0; left: 0; position: absolute; right: 0; top: 0; justify-content: space-between; align-items: center; display: flex; pointer-events: none; }
.dt-sc-chevronBtn { display: flex; border: none; border-radius: 50%; cursor: pointer; justify-content: center; align-items: center; backdrop-filter: var(--chevrons-button-backdrop-filter, none); background: transparent; background-color: var(--chevrons-button-color, var(--background-elevated-base)); height: 24px; opacity: 0; position: relative; transition: color .15s ease-out, opacity .15s ease-out, background-color .15s ease-out, translate .15s ease-out; translate: 0; width: 24px; z-index: 3; pointer-events: none; color: var(--text-base, #fff); }
.dt-sc-chevronBtn > * { opacity: .7; z-index: 2; }
.dt-sc-chevronBtn:hover { background-color: var(--chevrons-button-hover-color, var(--background-elevated-highlight)); }
.dt-sc-chevronBtn:hover > * { opacity: 1; }
.dt-sc-chevronBtn.dt-sc-chevronVisible { opacity: 1; pointer-events: auto; }
.dt-sc-onHover .dt-sc-chevronBtn { opacity: 0; }
.dt-sc-contentArea:hover .dt-sc-onHover .dt-sc-chevronBtn.dt-sc-chevronVisible { opacity: 1; }
.dt-sc-contentArea:hover .dt-sc-onHover .dt-sc-chevronStart.dt-sc-chevronVisible { translate: 8px; }
.dt-sc-contentArea:hover .dt-sc-onHover .dt-sc-chevronEnd.dt-sc-chevronVisible { translate: -8px; }
.dt-sc-scroller > div[role="presentation"] > button { margin-inline-start: 0px !important; }
body[data-dragging-uri-type] .dt-sc-chevronBtn { pointer-events: none; }`;
			document.head.appendChild(style);
		}

		function useDragToScroll({ isDisabled = true } = {}) {
			const { useRef, useCallback } = DT.React;
		...
		function useScrollState(scrollerRef, contentRef) {
			const { useState, useCallback, useEffect } = DT.React;
		...
		function ScrollableContainerComponent(props) {
			const { useRef, useCallback, useMemo } = DT.React;
			const h = DT.ReactJSX.jsx;
			const hsf = DT.ReactJSX.jsxs;
			const cn = DT.classnames;
			...
			const makeChevron = (svgPath, position, visible, dir) =>
				h("div", {
					className: cn("dt-sc-chevronBtn", `dt-sc-chevron${position}`, { "dt-sc-chevronVisible": visible }),
					onClick: (e) => {
						e.preventDefault();
						e.stopPropagation();
						navigate(dir);
			...
			return hsf("div", {
				className: cn("dt-sc-contentArea", className, {
					"dt-sc-linearGradient": isLinearGradient,
					"dt-sc-lgStart": isLinearGradient && canGoStart,
					"dt-sc-lgEnd": isLinearGradient && canGoEnd,
				}),
				children: [
					h("div", {
						ref: scrollerRef,
						className: cn("dt-sc-scroller", scrollerClassName, {
							"dt-sc-snap": isSnap,
							"dt-sc-maskGradient": isMask,
							"dt-sc-wheelEnabled": wheelScrollEnabled,
							"dt-sc-maskStart": isMask && canGoStart,
							"dt-sc-maskEnd": isMask && canGoEnd,
						}),
						...
						children: h("div", {
							...
							className: cn(scrollContentClassName, {
								"dt-sc-snapStart": scrollSnapAlign === "start",
								"dt-sc-snapCenter": scrollSnapAlign === "center",
							}),
							children,
						}),
					}),
					showButtons !== SHOW_BUTTONS.NEVER &&
						hsf("div", {
							className: cn("dt-sc-carousel", chevronsClassName, {
								"dt-sc-onHover": showButtons === SHOW_BUTTONS.ON_HOVER,
							}),
	...
	Object.assign(DT, {
		...
		Player: {
			...DT.Player,
			get origin() {
				return DT.Platform?.PlayerAPI;
			},
		},
		GraphQL: {
			...DT.GraphQL,
			get Request() {
				return DT.Platform?.GraphQLLoader || DT.GraphQL.Handler?.(DT.GraphQL.Context);
			},
	...
	if (!DT.ContextMenuV2._context) DT.ContextMenuV2._context = DT.React.createContext({});
	...
	(function waitForChunks() {
		...
		if (listOfComponents.every((component) => component.split(".").reduce((o, k) => o?.[k], DT.ReactComponent) !== undefined)) return;
		...
		Object.assign(DT.ReactComponent.Cards, Object.fromEntries(lazyCards));

		DT.ReactComponent.Slider = wrapProvider(functionModules.find((m) => fnStr(m).includes("progressBarRef")));
		DT.ReactComponent.Toggle = functionModules.find((m) => fnStr(m).includes("onSelected") && fnStr(m).includes('type:"checkbox"'));

		// chunks
		const dropdownChunk = currentChunks.find(([, value]) => fnStr(value).includes("dropdown-list") && fnStr(value).includes('"listbox"'));
		if (dropdownChunk) {
			DT.ReactComponent.Dropdown =
		...
		if (toggleChunk && !DT.ReactComponent.Toggle) {
			DT.ReactComponent.Toggle = Object.values(require(toggleChunk[0]))[0].render;
		}

		if (!listOfComponents.every((component) => component.split(".").reduce((o, k) => o?.[k], DT.ReactComponent) !== undefined)) {
			setTimeout(waitForChunks, 100);
			return;
		}

		if (DT.ReactComponent.ScrollableContainer) setTimeout(refreshNavLinks, 100);
	})();

	(function waitForSnackbar() {
		if (!Object.keys(DT.Snackbar).length) {
	...
	if (DT.Locale) DT.Locale._supportedLocales = cache.find((m) => typeof m?.ja === "string");

	Object.defineProperty(DT, "Queue", {
		get() {
			return DT.Player.origin?._queue?._state ?? DT.Player.origin?._queue?._queue;
		},
	});
	...
	if (!DT.ReactComponent?.ConfirmDialog && confirmDialogChunk) {
		DT.ReactComponent.ConfirmDialog = Object.values(require(confirmDialogChunk[0])).find((m) => typeof m === "object");
	} else {
		DT.ReactComponent.ConfirmDialog = functionModules.find(
	...
	if (contextMenuChunk) {
		DT.ReactComponent.ContextMenu = Object.values(require(contextMenuChunk[0])).find((m) => typeof m === "function");
	}
	...
	if (playlistMenuChunk && !DT.ReactComponent?.PlaylistMenu) {
		DT.ReactComponent.PlaylistMenu = Object.values(require(playlistMenuChunk[0])).find(
	...
	if (infiniteQueryChunk) {
		DT.ReactQuery.useInfiniteQuery = Object.values(require(infiniteQueryChunk[0])).find((m) => typeof m === "function");
	}

	if (DT.Color) DT.Color.CSSFormat = modules.find((m) => m?.RGBA);

	// Combine snackbar and notification
	(function bindShowNotification() {
		if (!DT.Snackbar?.enqueueSnackbar && !DT.showNotification) {
			setTimeout(bindShowNotification, 250);
			return;
		}

		if (DT.Snackbar?.enqueueSnackbar) {
			DT.showNotification = (message, isError, msTimeout) => {
				DT.Snackbar.enqueueSnackbar(message, {
		...
		DT.Snackbar.enqueueSnackbar = (message, { variant = "default", autoHideDuration } = {}) => {
			isError = variant === "error";
			DT.showNotification(message, isError, autoHideDuration);
		};
	})();

	// Image color extractor
	(async function bindColorExtractor() {
		if (!DT.GraphQL.Request) {
		...
		DT.extractColorPreset = async (image) => {
			const analysis = await imageAnalysis(DT.GraphQL.Request, image);
	...
	function wrapProvider(component) {
		...
		return (props) =>
			DT.React.createElement(
				DT.ReactComponent.RemoteConfigProvider,
				{ configuration: DT.Platform.RemoteConfiguration },
				DT.React.createElement(component, props)
			);
	}
	...
	(function waitForURI() {
		...
		// URI.Type
		DT.URI.Type = URIModules.find((m) => m?.PLAYLIST_V2);

		// Parse functions
		DT.URI.from = URIModules.find((m) => typeof m === "function" && m.toString().includes("allowedTypes"));
		DT.URI.fromString = URIModules.find((m) => typeof m === "function" && m.toString().includes("Argument `uri`"));

		// createURI functions
		const createURIFunctions = URIModules.filter((m) => typeof m === "function" && m.toString().match(/\([\w$]+\./));
		for (const type of Object.keys(DT.URI.Type)) {
		...
		for (const type of Object.keys(DT.URI.Type)) {
			...
			DT.URI[`is${camelCaseType}`] =
				func ??
				((uri) => {
					let uriObj;
					try {
						uriObj = DT.URI.from?.(uri) ?? DT.URI.fromString?.(uri);
					} catch {
						return false;
					}
					if (!uriObj) return false;
					return uriObj.type === DT.URI.Type[type];
				});
		}

		DT.URI.isPlaylistV1OrV2 = (uri) => DT.URI.isPlaylist(uri) || DT.URI.isPlaylistV2(uri);

		// Conversion functions
		DT.URI.idToHex = URIModules.find((m) => typeof m === "function" && m.toString().includes("22==="));
		DT.URI.hexToId = URIModules.find((m) => typeof m === "function" && m.toString().includes("32==="));

		// isSameIdentity
		DT.URI.isSameIdentity = URIModules.find((m) => typeof m === "function" && m.toString().match(/[\w$]+\.id===[\w$]+\.id/));
	})();

	DT.Events.webpackLoaded.fire();
	refreshNavLinks?.();
})();
...
// Wait for DT.Player.origin._state before adding following APIs
(function waitOrigins() {
	if (!DT?.Player?.origin?._state) {
	...
	const interval = setInterval(() => {
		if (!DT.Player.origin._state?.item) return;
		DT.Player.data = DT.Player.origin._state;
		playerState.cache = DT.Player.data;
		clearInterval(interval);
	}, 10);

	DT.Player.origin._events.addListener("update", ({ data: playerEventData }) => {
		playerState.current = playerEventData.item ? playerEventData : null;
		DT.Player.data = playerState.current;

		if (playerState.cache?.item?.uri !== playerState.current?.item?.uri) {
			const event = new Event("songchange");
			event.data = DT.Player.data;
			DT.Player.dispatchEvent(event);
		}
	...
	(function waitProductStateAPI() {
		if (!DT.Platform?.UserAPI) {
			setTimeout(waitProductStateAPI, 100);
			return;
		}

		const productState = DT.Platform.UserAPI._product_state || DT.Platform.UserAPI._product_state_service;
		if (productState) return;
		if (!DT.Platform?.ProductStateAPI) {
			setTimeout(waitProductStateAPI, 100);
			return;
		}

		const productStateApi = DT.Platform.ProductStateAPI.productStateApi;
		DT.Platform.UserAPI._product_state_service = productStateApi;
	})();

	(async function setButtonsHeight() {
		while (!DT.CosmosAsync) {
			await new Promise((res) => setTimeout(res, 100));
		}
		const expFeatures = JSON.parse(localStorage.getItem("dt-exp-features") || "{}");
		const isGlobalNavbar = expFeatures?.enableGlobalNavBar?.value;

		if (typeof isGlobalNavbar !== "undefined" && isGlobalNavbar === "control") {
			await DT.CosmosAsync.post("sp://messages/v1/container/control", {
				type: "update_titlebar",
				height: DT.Platform.PlatformData.os_name === "osx" ? "42" : "40",
			});
		}
	})();
	...
	setInterval(() => {
		if (playerState.cache?.isPaused === false) {
			...
			event.data = DT.Player.getProgress();
			DT.Player.dispatchEvent(event);
		}
	}, 100);

	DT.addToQueue = (uri) => {
		return DT.Player.origin._queue.addToQueue(uri);
	};
	DT.removeFromQueue = (uri) => {
		return DT.Player.origin._queue.removeFromQueue(uri);
	};
})();

DT.getAudioData = async (uri) => {
	const providedURI = uri || DT.Player.data.item.uri;
	const uriObj = DT.URI.from?.(providedURI) ?? DT.URI.fromString?.(providedURI);
	if (!uriObj || (uriObj.Type || uriObj.type) !== DT.URI.Type.TRACK) {
		throw "URI is invalid.";
	}

	return await DT.CosmosAsync.get(
		`https://spclient.wg.spotify.com/audio-attributes/v1/audio-analysis/${uriObj.getBase62Id?.() ?? uriObj.id}?format=json`
	);
};

DT.colorExtractor = async (uri) => {
	const body = await DT.CosmosAsync.get(`https://spclient.wg.spotify.com/colorextractor/v1/extract-presets?uri=${uri}&format=json`);
...
DT._getStyledClassName = (args, component) => {
	const includedKeys = [
		"role",
		"variant",
		"semanticColor",
...
(function waitMouseTrap() {
	...
	function formatKeys(keys) {
		...
		if (typeof keys === "object") {
			if (!keys.key || !Object.values(KEYS).includes(keys.key)) {
				throw `DT.Keyboard.registerShortcut: Invalid key ${keys.key}`;
			}
			...
			throw `DT.Keyboard.registerShortcut: Invalid keys ${keys}`;
		}
		return keystroke;
	}
	...
	DT.Keyboard = {
		...
		registerShortcut: (keys, callback) => {
			DT.Mousetrap.bind(formatKeys(keys), callback);
		},
		_deregisterShortcut: (keys) => {
			DT.Mousetrap.unbind(formatKeys(keys));
		},
		changeShortcut: (keys, newKeys) => {
			if (!keys || !newKeys) throw "DT.Keyboard.changeShortcut: Invalid keys";

			const callback = Object.keys(DT.Mousetrap.trigger()._directMap).find((key) => key.startsWith(formatKeys(keys)));
			if (!callback) throw "DT.Keyboard.changeShortcut: Shortcut not found";

			DT.Keyboard.registerShortcut(newKeys, DT.Mousetrap.trigger()._directMap[callback]);
			DT.Keyboard._deregisterShortcut(keys);
		},
	};
	DT.Keyboard.registerIsolatedShortcut = DT.Keyboard.registerShortcut;
	DT.Keyboard.registerImportantShortcut = DT.Keyboard.registerShortcut;
	DT.Keyboard.deregisterImportantShortcut = DT.Keyboard._deregisterShortcut;
})();
...
(async function waitUserAPI() {
	...
	const productState =
		DT.Platform.UserAPI?._product_state ||
		DT.Platform.UserAPI?._product_state_service ||
		DT.Platform?.ProductStateAPI.productStateApi;
...
function parseIcon(icon, size = 16) {
	if (icon && DT.SVGIcons[icon]) {
		return `<svg height="${size}" width="${size}" viewBox="0 0 ${size} ${size}" fill="currentColor">${DT.SVGIcons[icon]}</svg>`;
	}
	return icon || "";
}

function createIconComponent(icon, size = 16) {
	return DT.React.createElement(
		DT.ReactComponent.IconComponent,
		{
			iconSize: size,
...
DT.ContextMenuV2 = (() => {
	...
	class Item {
		constructor({ children, disabled = false, leadingIcon, trailingIcon, divider, onClick, shouldAdd = () => true }) {
			...
			this._element = DT.ReactJSX.jsx(() => {
				const [_children, setChildren] = DT.React.useState(this._children);
				const [_disabled, setDisabled] = DT.React.useState(this._disabled);
				const [_leadingIcon, setLeadingIcon] = DT.React.useState(this._leadingIcon);
				const [_trailingIcon, setTrailingIcon] = DT.React.useState(this._trailingIcon);
				const [_divider, setDivider] = DT.React.useState(this._divider);
				...
				const context = DT.React.useContext(DT.ContextMenuV2._context) ?? {};

				return DT.React.createElement(DT.ReactComponent.MenuItem, {
					disabled: _disabled,
					divider: _divider,
					onClick: (e) => {
		...
		register() {
			DT.ContextMenuV2.registerItem(this._element, this.shouldAdd);
		}
		deregister() {
			DT.ContextMenuV2.unregisterItem(this._element);
		}
	}
	...
	class ItemSubMenu {
		...
		constructor({ text, disabled = false, leadingIcon, divider, items, depth = 1, shouldAdd = () => true }) {
			...
			this._depth = depth;
			this._element = DT.ReactJSX.jsx(() => {
				const [_text, setText] = DT.React.useState(this._text);
				const [_disabled, setDisabled] = DT.React.useState(this._disabled);
				const [_leadingIcon, setLeadingIcon] = DT.React.useState(this._leadingIcon);
				const [_divider, setDivider] = DT.React.useState(this._divider);
				const [_items, setItems] = DT.React.useState(this._items);
				const [_depth, setDepth] = DT.React.useState(this._depth);
				...
				return DT.React.createElement(DT.ReactComponent.MenuSubMenuItem, {
					displayText: _text,
					divider: _divider,
					depth: _depth,
	...
	const renderItems = () => {
		const { props, trigger, target } = DT.React.useContext(DT.ContextMenuV2._context) ?? {};
...
DT.Menu = (() => {
	...
	class Item extends DT.ContextMenuV2.Item {
	...
	class SubMenu extends DT.ContextMenuV2.ItemSubMenu {
...
DT.ContextMenu = (() => {
	const iconList = Object.keys(DT.SVGIcons);
	...
	class Item extends DT.ContextMenuV2.Item {
		...
		constructor(name, onClick, shouldAdd = () => true, icon = undefined, trailingIcon = undefined, disabled = false) {
			super({
				...
				trailingIcon,
				onClick: (context) => {
					const [uris, uids, contextUri] = DT.ContextMenuV2.parseProps(context.props);
					onClick(uris, uids, contextUri);
				},
				shouldAdd: (props) => {
					const parsedProps = DT.ContextMenuV2.parseProps(props);
...
DT._renderNavLinks = (list, isTouchScreenUi) => {
	const [, refresh] = DT.React.useReducer((x) => x + 1, 0);
	refreshNavLinks = refresh;

	if (
		!DT.ReactComponent.ButtonTertiary ||
		!DT.ReactComponent.Navigation ||
		!DT.ReactComponent.TooltipWrapper ||
		!DT.ReactComponent.ScrollableContainer ||
		!DT.Platform.History ||
		!DT.Platform.LocalStorageAPI
	)
		return;

	const navLinkFactory = isTouchScreenUi ? NavLinkGlobal : NavLinkSidebar;

	if (!navLinkFactoryCtx) navLinkFactoryCtx = DT.React.createContext(null);
	const registered = [];
	...
	for (const app of list) {
		...
		try {
			...
			request.open("GET", `dt-routes-${app}.json`, false);
			request.send(null);
		...
		if (typeof appProper === "object") {
			appProper = appProper[DT.Locale?.getLocale()] || appProper.en;
		}
		if (!appProper) {
	...
	(function addStyling() {
		if (document.querySelector("style.dt-navlinks")) return;
		const style = document.createElement("style");
		style.className = "dt-navlinks";
		style.innerHTML = `
	:root {
	...
	const wrapScrollableContainer = (element) =>
		DT.React.createElement(
			...
			DT.React.createElement(DT.ReactComponent.ScrollableContainer, null, element)
		);
	...
	const NavLinks = () =>
		DT.React.createElement(
			...
			registered.map((NavLinkElement) => DT.React.createElement(NavLink, NavLinkElement, null))
		);
...
const NavLink = ({ appProper, appRoutePath, icon, activeIcon }) => {
	const isActive = DT.Platform.History.location.pathname?.startsWith(appRoutePath);
	const createIcon = () => createIconComponent(isActive ? activeIcon : icon, 24);

	const NavLinkFactory = DT.React.useContext(navLinkFactoryCtx);

	return NavLinkFactory && DT.React.createElement(NavLinkFactory, { appProper, appRoutePath, createIcon, isActive }, null);
};

const NavLinkSidebar = ({ appProper, appRoutePath, createIcon, isActive }) => {
	const isSidebarCollapsed = DT.Platform.LocalStorageAPI.getItem("ylx-sidebar-state") === 1;
		...
		DT.React.createElement(
			...
			DT.React.createElement(
				DT.ReactComponent.Navigation,
				{
					to: appRoutePath,
					referrer: "other",
					className: DT.classnames("link-subtle", "main-yourLibraryX-navLink", {
				...
				createIcon(),
				!isSidebarCollapsed && DT.React.createElement(DT.ReactComponent.TextComponent, { variant: "balladBold" }, appProper)
			)
		)
	);
};
...
const NavLinkGlobal = ({ appProper, appRoutePath, createIcon, isActive }) => {
	return DT.React.createElement(
		DT.ReactComponent.TooltipWrapper,
		{ label: appProper },
		DT.React.createElement(DT.ReactComponent.ButtonTertiary, {
			iconOnly: createIcon,
			className: DT.classnames("link-subtle", "main-globalNav-navLink", "main-globalNav-link-icon", "custom-navlink", {
				"main-globalNav-navLinkActive": isActive,
			}),
			"aria-label": appProper,
			onClick: () => DT.Platform.History.push(appRoutePath),
		})
	);
};
...
class _HTMLGenericModal extends HTMLElement {
	hide() {
		DT.ReactDOM.unmountComponentAtNode(this.querySelector("main"));
		this.remove();
	}
...
<div class="GenericModal__overlay" style="z-index: 100;">
	<div class="GenericModal" tabindex="-1" role="dialog" aria-label="${title}" aria-modal="true">
		...
		if (DT.React.isValidElement(content)) {
			DT.ReactDOM.render(content, main);
...
DT.PopupModal = new _HTMLGenericModal();

Object.defineProperty(DT, "TippyProps", {
	value: {
		delay: [200, 0],
		animation: true,
		render(instance) {
...
DT.Topbar = (() => {
	...
	class Button {
		constructor(label, icon, onClick, disabled = false, isRight = false) {
			...
			this.tippy = DT.Tippy?.(this.element, {
				content: label,
				...DT.TippyProps,
			});
			this.label = label;
		...
		set icon(input) {
			let newInput = input;
			if (newInput && DT.SVGIcons[newInput]) {
				newInput = `<svg height="16" width="16" viewBox="0 0 16 16" fill="currentColor">${DT.SVGIcons[newInput]}</svg>`;
			}
			this._icon = newInput;
	...
	(function waitForPlatform() {
		if (!DT.Platform?.History) {
			setTimeout(waitForPlatform, 100);
			return;
		}

		DT.Platform.History.listen(() => waitForTopbarMounted());
	})();
...
DT.Playbar = (() => {
	...
	class Button {
		...
		set icon(input) {
			let newInput = input;
			if (newInput && DT.SVGIcons[newInput]) {
				newInput = `<svg height="16" width="16" viewBox="0 0 16 16" fill="currentColor" stroke="currentColor">${DT.SVGIcons[newInput]}</svg>`;
			}
			this._icon = newInput;
...
(async function checkForUpdate() {
	...
	const { check_dt_update, version } = DT.Config;
	// Skip checking if upgrade check is disabled, or version is Dev/version is not set
	if (!check_dt_update || !version || version === "Dev") return;
	// Fetch latest version from GitHub
	try {
		let changelog;
		const res = await fetch("https://api.github.com/repos/DT-Deville/DT/releases/latest");
		...
		if (semver !== version) {
			...
			content.innerHTML = `
				<style>
					...
					.dt-update-little-space {
						margin-bottom: 8px;
					}
				</style>
				<p class="dt-update-space">Update DT to receive new features and bug fixes.</p>
				...
				<ol>
					<li>Update DT CLI</li>
					<pre class="dt-update-little-space">dt update</pre>
					<p>DT will automatically apply changes to Spotify after upgrading to the latest version.</p>
					<p>If you installed DT via a package manager, update using said package manager.</p>
				</ol>
			`;
			...
			(function waitForTippy() {
				...
				const tippy = DT.Tippy(content.querySelectorAll("pre"), {
				...
				for (const instance of tippy) {
					instance.reference.addEventListener("click", () => {
						DT.Platform.ClipboardAPI.copy(instance.reference.textContent);
			...
			new DT.Topbar.Button(
				...
				() => DT.PopupModal.display(updateModal)
			);
		}
	} catch (err) {
		console.error(err);
